...
try {
  ...
  console.error(
    chalk.red(`Error loading dashboard template: ${error.message}`),
  );
...
try {
  ...
  console.warn(
    chalk.yellow(
      `Warning: JavaScript in HTML template may have errors: ${error.message}`,
    ),
  );
}
  ...
  try {
    ...
    if (fs.existsSync(configPath)) {
      ...
      if (featuresMatch) {
        ...
        if (featuresItems) {
          info.features = featuresItems.map((item) =>
            item.replace(/['"]/g, ""),
          );
        }
      }
    ...
    console.error(
      chalk.red(`Error getting plugin config info: ${error.message}`),
    );
...
function startPlugin() {
  ...
  // Execute the plugin
  pluginProcess = exec(
    `cd ${PLUGIN_DIR} && bun run src/index.js`,
  ...
  if (pluginProcess) {
    pluginProcess.on("exit", (code: number | null, signal: string | null) => {
      ...
      const duration =
        pluginMetadata.endTime && pluginMetadata.startTime
          ? (pluginMetadata.endTime.getTime() -
              pluginMetadata.startTime.getTime()) /
            1000
          : 0; // in seconds

      console.log(
        chalk.yellow(
          `Plugin process exited with code ${code}${signal ? `, signal: ${signal}` : ""}`,
        ),
      );
      io.emit("plugin-log", {
...
function triggerEvent(eventName: string, payload: any) {
  ...
  io.emit("plugin-log", {
    message: `Event payload: ${JSON.stringify(payload, null, 2)}`,
    type: "info",
  });
}
...
server.listen(PORT, () => {
  console.log(
    chalk.green(`Development server running at http://localhost:${PORT}`),
  );
  console.log(chalk.green(`Plugin directory: ${PLUGIN_DIR}`));
  console.log(chalk.green(`Default event: ${EVENT_NAME}`));
  console.log(
    chalk.green(`Watch mode: ${WATCH_MODE ? "enabled" : "disabled"}`),
  );
