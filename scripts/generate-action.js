...
async function loadConfig() {
  if (fs.existsSync(jsConfigPath)) {
    ...
    console.error('No plugin configuration found. Please create plugin.config.js or plugin.config.ts');
    process.exit(1);
  }
}
...
async function generateAction() {
  try {
    ...
    const actionConfig = {
      ...
      ...(pluginConfig.action.outputs && Object.keys(pluginConfig.action.outputs).length > 0 ? {
