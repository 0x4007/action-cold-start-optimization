...
const analyzePullRequest = (
  ...
  const hasDocumentation =
    pull_request.body?.toLowerCase().includes("documentation") ||
    pull_request.body?.toLowerCase().includes("docs") ||
    false;
 ...
 */
const handlePullRequestOpened: EventHandler<PullRequestOpenedPayload> = async (
  payload,
) => {
  ...
  const sizeThresholdsJson =
    process.env.INPUT_SIZETHRESHOLDS ||
    '{"small": 10, "medium": 100, "large": 500, "xlarge": 1000}';
